...
module MailControllerTestHelper
  ...
  def deliver(action, mail_params = {}, send_params = {})
    NotificationsMailer.dispatch_and_deliver(action, { :from => "no-reply@webapp.com", :to => "recepient@person.com" }.merge(mail_params), send_params)
    @delivery = last_delivered_mail
  end
end

describe NotificationsMailer, "#notify_on_event email template" do
